...
private[sql] case class CarbonDescribeFormattedCommand(
  ...
  override def processMetadata(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (isLocalDictEnabled.toBoolean) {
      ...
      if (tblProps.asScala
        .get(CarbonCommonConstants.LOCAL_DICT_INCLUDE).isDefined) {
        val allLocalDictColumns = tblProps.asScala(CarbonCommonConstants.LOCAL_DICT_INCLUDE)
          .split(",")
        results ++= Seq(("Local Dictionary Include", getDictColumnString(allLocalDictColumns), ""))
      }
      if (tblProps.asScala
        .get(CarbonCommonConstants.LOCAL_DICT_EXCLUDE).isDefined) {
        val allLocalDictColumns = tblProps.asScala(CarbonCommonConstants.LOCAL_DICT_EXCLUDE)
          .split(",")
